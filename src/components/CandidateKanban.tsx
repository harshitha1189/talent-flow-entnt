...
const stages = [
  ...
  { key: 'Interviewing' as const, label: 'Screening Interview', color: 'text-yellow-400' },
  { key: 'Interviewing' as const, label: 'Technical Assessment', color: 'text-orange-400' },
  { key: 'Offered' as const, label: 'Offer Sent', color: 'text-blue-400' },
];
...
export default function CandidateKanban({ candidate, onStageChange }: CandidateKanbanProps) {
  ...
  // Find the column based on candidate stage & timeline
  const getColumnForStage = () => {
    if (candidate.stage === 'Applied') return 0;
    if (candidate.stage === 'Interviewing') {
      const lastLabel = candidate.timeline[candidate.timeline.length - 1]?.label;
      return lastLabel === 'Technical Assessment' ? 2 : 1;
    }
    if (candidate.stage === 'Offered') return 3;
    return 0;
  };
                  ...
                  <div className="mt-3 flex gap-1">
                    ...
                    {index > 0 && (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => {
                          const prevStage: { stage: Candidate['stage']; label: string } =
                            index === 1
                              ? { stage: 'Applied', label: 'Application Received' }
                              : { stage: 'Interviewing', label: 'Screening Interview' };

                          onStageChange(prevStage.stage, prevStage.label);
                        }}
                    ...
                    {index < stages.length - 1 && (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => {
                          const nextStage: { stage: Candidate['stage']; label: string } =
                            index === 0
                              ? { stage: 'Interviewing', label: 'Screening Interview' }
                              : index === 1
                              ? { stage: 'Interviewing', label: 'Technical Assessment' }
                              : { stage: 'Offered', label: 'Offer Sent' };

                          onStageChange(nextStage.stage, nextStage.label);
                        }}
